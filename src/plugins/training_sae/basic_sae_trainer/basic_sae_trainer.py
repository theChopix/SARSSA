...
from utils.torch.models.sae_model.basic_sae import BasicSAE
...
def train(
    ...
    """Train a Basic (L1) Sparse Autoencoder with early stopping and MLflow tracking.
    ...
    def train_epoch(epoch, d):
        ...
        def step(anchor, pbar):
            ...
            losses = model.train_step(optimizer, anchor, None)
    ...
    def evaluate(epoch, d):
        ...
        valid_batch_rows = []
        for embedding in d["valid_embeddings_dataloader"]:
            losses = model.compute_loss_dict(embedding, None)
...
class Plugin(BasePlugin):
    """Basic (L1) SAE training plugin.

    Trains a Basic sparse autoencoder on top of a pre-trained base model. Sparsity is
    driven by a soft L1 penalty (``l1_coef``) rather than a hard active-feature budget,
    learning interpretable, sparse representations of user embeddings while maintaining
    ...
    description = (
        ...
        "penalty (l1_coef) instead of a hard top_k budget. Each neuron ideally captures one "
        "interpretable concept while reconstruction preserves recommendation quality."
    )
    ...
    def run(
        ...
        l1_coef: Annotated[
            float,
            "Strength of the L1 sparsity penalty on activations. The primary "
            "sparsity control for BasicSAE: higher is sparser and more "
            "interpretable but reconstructs worse. Default 3e-4.",
        ] = 3e-4,
        ...
        """Execute the Basic SAE training pipeline.
        ...
        Args:
            ...
            l1_coef: L1 sparsity penalty coefficient (primary sparsity control).
            evaluate_every: Evaluate every N epochs.
        ...
        self.notifier.info(
            f"Basic SAE training starting — {embedding_dim}d, {epochs} epochs, device={device}"
        )
        ...
        logger.info(f"Initialized BasicSAE with {embedding_dim} dimensions")
