...
class DatasetArtifacts:
    ...
    tag_item_matrix: Optional[sp.csr_matrix] = None
...
class DatasetArtifactLoader:
    ...
    def from_context(cls, context: dict) -> "DatasetArtifactLoader":
        ...
        if "dataset" not in context:
            raise ValueError("Context does not contain 'dataset' key. "
    ...
    def load_metadata(self) -> Dict[str, Any]:
        """Load metadata.json."""
        with open(f"{self.artifact_uri}/metadata.json", "r") as f:
            return json.load(f)
    ...
    def load_tag_ids(self) -> Optional[list]:
        """Load tag IDs (only available for MovieLens)."""
        try:
            with open(f"{self.artifact_uri}/tag_ids.json", "r") as f:
                return json.load(f)
        except FileNotFoundError:
            return None
    
    def load_tag_item_matrix(self) -> Optional[sp.csr_matrix]:
    ...
    def load_for_training(self) -> tuple[sp.csr_matrix, sp.csr_matrix, sp.csr_matrix, np.ndarray, np.ndarray]:
        """
    ...
    def load_for_neuron_labeling(self) -> tuple[sp.csr_matrix, np.ndarray, Optional[list], Optional[sp.csr_matrix]]:
        """
