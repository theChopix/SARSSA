...
def split_input_target_interactions(
    user_item_csr: sp.csr_matrix, target_ratio: float, seed: int = 42
...
def _recall_at_k_batch(
    batch_topk_indices: torch.Tensor, batch_target: torch.Tensor, k: int
) -> torch.Tensor:
    ...
    predicted_batch = torch.zeros_like(target).scatter(
        1, batch_topk_indices, torch.ones_like(batch_topk_indices, dtype=torch.bool)
    )
    r = (predicted_batch & target).sum(dim=1) / torch.minimum(
        target.sum(dim=1), torch.ones_like(target.sum(dim=1)) * k
    )
    return r
...
def ndcg_at_k(
    topk_batch: torch.Tensor, target_batch: torch.Tensor, k: int
...
def evaluate_dense_encoder(
    model: BaseModel,
    split_csr: sp.csr_matrix,
    target_ratio: float,
    batch_size: int,
    device,
    seed: int = 42,
...
def evaluate_cosine_similarity(
    embeddings: torch.Tensor, reconstructed_embeddings: torch.Tensor
) -> np.ndarray:
    ...
    """
    return (
        torch.nn.functional.cosine_similarity(embeddings, reconstructed_embeddings, dim=1)
        .detach()
        .cpu()
        .numpy()
    )
...
def evaluate_sparse_encoder(
    ...
    sparse_embeddings = torch.tensor(
        np.vstack(
            [sae_model.encode(batch)[0].detach().cpu().numpy() for batch in embeddings_dataloader]
        ),
        device=device,
    )
    ...
    recalls_with_sae = np.mean(
        np.concatenate(
            [
                _recall_at_k_batch(recs, targs, 20).cpu().numpy()
                for recs, targs in zip(sae_rec_dataloader, targets)
            ]
        )
    )
